...
from predicu.data import CUM_COLUMNS, NCUM_COLUMNS, load_bedcounts
from predicu.plot import COLUMN_TO_HUMAN_READABLE, COL_COLOR
...
def plot_icu_values(icu_name, data_preprocessed, path, data_raw):
    ...
    data_raw = (
        data_raw.loc[data_raw.icu_name == icu_name]
        .set_index("datetime")
        .sort_index()
    )
    for i, (cols_grp, ax_raw, ax_clean) in enumerate(
        ...
        for col in cols_grp:
            for j, (ax, df) in enumerate(
                [(ax_raw, data_raw), (ax_clean, data_preprocessed)]
            ):
                ax.plot(
                    df.index,
                    df[col],
